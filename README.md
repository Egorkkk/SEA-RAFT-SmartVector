...
**[Русская справка](docs/README.ru.md)** · [Release notes](CHANGELOG.md)
...
No WSL, virtual machine, system Python 3.10, or separate CUDA Toolkit installation is needed. You need a 64-bit Windows machine with an NVIDIA GPU and compatible driver, Git for Windows, PowerShell, internet access, and Nuke. The plugin has been tested with Nuke 17; other Nuke versions have not been verified.
...
Restart Nuke and create **SEA-RAFT → SEA-RAFT SmartVector** from the Nodes menu. Connect the image graph, click **Export Write**, set the new Write node's EXR sequence path with a frame token such as `####` or `%04d`, then click **Generate + Render** on the SmartVector node. The Write node outputs all channels as 32-bit float EXR. Existing Groups from the previous version are upgraded when a script opens, or with **SEA-RAFT → Upgrade Existing Nodes**.
...
- **Generate + Render** on the SmartVector node first uses a temporary Write to materialize the actual upstream image, runs the external worker, then renders the final output through the connected Write. Nuke cannot start another `nuke.execute` from a Write `beforeRender` callback. A normal render of the Write uses previously prepared vectors; after any input or settings change, use **Generate + Render** again. The callback rejects missing vectors or settings that no longer match.
...
The installer was run on native Windows with an RTX 4060 Ti and driver 595.79. A headless Nuke 17 test clicked the **Export Write** and **Generate + Render** knobs and passed the full two-frame render; the final EXR contained RGB and all 28 SmartVector channels as 32-bit float. `tests/verify_motion.py` checked a four-pixel horizontal shift and measured approximately +4 pixels forward and −4 pixels backward. A short shot should still be checked visually in stock `VectorDistort`. Render farm setup and color pipeline behavior have not been verified.
